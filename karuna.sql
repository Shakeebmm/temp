...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `spr_GetDealerNames` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `spr_GetDealerNames`()
BEGIN
	
   select distinct dealer from medicinepurchase order by dealer;
...
BEGIN
    ...
    p.`Temp`,p.`Route`,p.`HomeCarePlan`, p.`Category`
	from patient p
...
BEGIN
	...
	`RegDate`,`ExpDate`,`DropDate`,`Volunteer`,`Diagnosis`,`Temp`,`Route`,`HomeCarePlan`,`Category`
	from patient
	where Pno = p_in_pno;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `spr_ReportMedicineDealer` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `spr_ReportMedicineDealer`(
IN p_in_dealer varchar(100),
IN p_in_fromDate datetime,
IN p_in_toDate datetime
)
BEGIN

	IF p_in_dealer = "[No Dealer]" THEN
		select 
			p.id,
			m.name,
			p.invdate,
			...
			p.expdate,
			p.mfgdate,
			p.mainStock + p.subStock quantity
		from medicinepurchase p
		inner join medicine m on p.MedicineId = m.id
		where (p.dealer = '' OR p.dealer is null)  AND
			p.invdate >= p_in_fromDate AND
			p.invdate <= p_in_toDate
		order by p.invdate,m.name;
	ELSE
		select 
			p.id,
			m.name,
		...
		from medicinepurchase p
		inner join medicine m on p.MedicineId = m.id
		where p.dealer = p_in_dealer  AND
			p.invdate >= p_in_fromDate AND
			p.invdate <= p_in_toDate
		order by p.invdate,m.name;
	END IF;
...
-- Dump completed on 2023-05-06 18:36:54
